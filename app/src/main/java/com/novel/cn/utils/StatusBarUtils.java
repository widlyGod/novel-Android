...
public class StatusBarUtils {
    ...
    public static float DEFAULT_ALPHA = Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP ? 0f : 0.3f;
    ...
    public static void darkMode(Activity activity) {
        float alpha = Build.VERSION.SDK_INT >= Build.VERSION_CODES.M ? 0f : 0.3f;
        darkMode(activity.getWindow(), DEFAULT_COLOR, alpha);
    }
    ...
    public static void darkMode(Window window, int color, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        if (isFlyme4Later()) {
            ...
            window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            setTranslucentView((ViewGroup) window.getDecorView(), color, alpha);
            Log.w("==========Status ","KITKAT==============================>>>>>"+alpha);
        } else {
            Log.w("==========Status ","immersive==============================>>>>>"+alpha);
            immersive(window, color, alpha);
        }
//        if (Build.VERSION.SDK_INT >= 21) {
//            window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
