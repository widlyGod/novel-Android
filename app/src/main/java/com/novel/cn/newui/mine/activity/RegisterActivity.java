...
public class RegisterActivity extends AutoLayoutActivity implements LoginContract.View{
...
//    boolean isIntentRead=false;
    ...
    @OnClick({R.id.iv_left, R.id.iv_qq, R.id.iv_wx, R.id.iv_wb,
            R.id.tv_sendyzm,R.id.tv_yhzcxy,R.id.btn_LoginOrRes})
...
//        other.SSOSetting(false);//优先选择调用手机客户端界面   true是网页界面
        other.setPlatformActionListener(new PlatformActionListener() {
            ...
            public void onError(Platform platform, int i, Throwable throwable) {
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        if (typex == 0 && ix == 1) {
                            Toast.makeText(RegisterActivity.this, "qq客户端不存在", Toast.LENGTH_SHORT).show();
                        } else if (typex == 1 && ix == 8) {
                            Toast.makeText(RegisterActivity.this, "微信客户端不存在", Toast.LENGTH_SHORT).show();
                        } else {
        ...
        Observable.combineLatest(obetNicen, obetUsername2,obetPw2,obetConps,obetYzm,
            ...
            public void call(Boolean aBoolean) {
                ...
                if (aBoolean) {
                    ...
                    btn_LoginOrRes.setBackground(ContextCompat.getDrawable(RegisterActivity.this, R.drawable.shape_login_conbtn));
                    String userName = etUsername2.getText().toString();
                    String pw = etPw2.getText().toString();
                    String conps = etConps.getText().toString();
                    if (PartsUtil.isEmail(userName) == false ) {
                    ...
                    btn_LoginOrRes.setBackground(ContextCompat.getDrawable(RegisterActivity.this, R.drawable.shape_login_btn));
                    tv_messageregist.setText("");
                    errorText="";
                }
            }
    ...
    public void otherLoginResponse(BaseObjectBean<UserBean> data, String type, String opid, String sex, String face) {
        ...
        if(data.isSuccess()){
            SharePrefUtil.saveBoolean(RegisterActivity.this, "isLogin", true);
