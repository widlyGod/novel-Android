...
class PublishActivity : BaseActivity<PublishPresenter>(), PublishContract.View {
    ...
    override fun initData(savedInstanceState: Bundle?) {
        ...
        ll_location_selected_show.clicks().subscribe {
            ...
            ll_location_selected_show.visible(false)
            locationTitle = ""
            longitude = 0.0
            latitude = 0.0
        }.bindToLifecycle(this)
    ...
    @Subscribe(threadMode = ThreadMode.MAIN)
    fun onPoiItemEvent(event: PoiItem) {
        ll_location_select.visible(false)
        ll_location_selected_show.visible(true)
        tv_location_name.text = event.title
        locationTitle = event.title
        longitude = event.latLonPoint.longitude
        latitude = event.latLonPoint.latitude
    }
}
